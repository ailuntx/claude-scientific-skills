...
md = MarkItDown(
    ...
    llm_prompt="描述此科研幻灯片，重点关注数据和关键发现"
)
...
md = MarkItDown(
    llm_client=client,
    llm_model="anthropic/claude-sonnet-4.5",  # 推荐用于科学图像识别
...
def safe_convert(filepath):
    ...
    try:
        ...
        logger.error(f"文件格式无效: {filepath}: {e}")
        return False
