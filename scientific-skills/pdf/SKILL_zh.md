---
name: pdf
description: 当用户需要对 PDF 文件进行任何操作时使用此技能。包括读取或从 PDF 中提取文本/表格、合并多个 PDF 为一个、拆分 PDF、旋转页面、添加水印、创建新 PDF、填写 PDF 表单、加密/解密 PDF、提取图像以及对扫描的 PDF 进行 OCR 使其可搜索。如果用户提到 .pdf 文件或要求生成一个，请使用此技能。
...
本指南涵盖了使用 Python 库和命令行工具进行基本 PDF 处理操作。有关高级功能、JavaScript 库以及详细示例，请参阅 reference.md。如果您需要填写 PDF 表单，请阅读 forms.md 并按照其说明操作。
...
请改用 ReportLab 在 Paragraph 对象中的 XML 标记：
```python
...
对于 canvas 绘制的文本（而非 Paragraph 对象），请手动调整字体大小和位置，而不是使用 Unicode 下标/上标。
